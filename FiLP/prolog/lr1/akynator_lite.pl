...
apple_only(swift, 1).
apple_only(java, 0).
apple_only(c_sharp, 0).
apple_only(ruby, 0).
apple_only(python, 0).
apple_only(f_sharp, 0).
apple_only(prolog, 0).
apple_only(c, 0).
apple_only(asm, 0).
apple_only(c_plu_plus, 0).
...
question8(X8):- write("Is your language designed mainly for Apple platforms?"), nl,
                write("1. Yes"), nl,
                write("0. NO"), nl,
                read(X8).				
...
pr:-	question1(X1),question2(X2),question3(X3),question4(X4),
		...
		cross(X,X5),visual(X,X6),mobile(X,X7), apple_only(X,X8),
		write(X).
